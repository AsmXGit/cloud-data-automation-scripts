...
if [ "$OS" == RedHatEnterpriseServer ] || [ "$OS" == CentOS ]; then
  ...
  if [ -d /etc/sysctl.d ]; then
    ...
    echo "# AllOps" >>/etc/sysctl.d/cloudera-ipv6.conf
...
# Hadoop: Disable IPv6 support # AllOps
AddressFamily inet             # AllOps
ListenAddress 0.0.0.0          # AllOps
# Hadoop: Disable IPv6 support # AllOps
EOF
service ssh restart
